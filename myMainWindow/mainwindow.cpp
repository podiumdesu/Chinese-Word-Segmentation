    ...
    ui(new Ui::MainWindow)
{
    ...
    //修改背景图
    QPixmap bkgnd("/Users/petnakanojo/myMainWindow/11.png");
...
void MainWindow::getReadFromFile(char * resolveFilePath, int saveOrNot, char * saveFile) {
    ...
    t.open(resolveFilePath);      // open input file
    t.seekg(0, std::ios::end);    // go to the end
    length = t.tellg();           // report location (this is the length)
    t.seekg(0, std::ios::beg);    // go back to the beginning
    t.read(buffer, length);       // read the whole file into the buffer
...
void MainWindow::on_getTextEdit_clicked()
{
    int saveOrNot = 1;
...
void MainWindow::on_removeTextEdit_clicked()
{
    ui->textEdit->setText("");
